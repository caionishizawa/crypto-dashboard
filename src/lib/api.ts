  ...
  async createCliente(clienteData: any): Promise<ApiResponse> {
    try {
      ...
      const clienteCompleto = {
        ...
        // Campo obrigatório - usar o ID do usuário dono do perfil se fornecido; senão, fallback para o logado
        usuarioId: clienteData.usuarioId || user.id,
  ...
  async transformarUsuarioEmAdmin(usuarioId: string): Promise<ApiResponse> {
    try {
      ...
      if (fetchError || !updatedUser) {
        console.error('🔧 API - Erro ao verificar usuário atualizado:', fetchError);
        return { success: false, error: 'Erro ao verificar se a atualização foi bem-sucedida' };
      }
